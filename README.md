...
- 冻结前可以新增发布范围。冻结后不能升级为 `release`，已在发布范围内的 app_info 更新也不能扩大受控 QA 范围；只有 QA 例外：接口先返回修改点和新增范围不写库，QA 确认接受后才写入，并记一条 `qa_accept_scope_expansion` 审计。RM 和 Owner 仍然被直接拒绝。
