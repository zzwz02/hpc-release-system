...
- HPC manual artifact: `artifacts.kind = "manual"` / `hpc_manual_apps.md` -> `HPC_Manual_CN.rst` chapter 10.
...
4. Resolve conflicts:
   ...
   - For entries present in RST but absent from the latest DB artifact, keep them only when they are clearly general/manual content rather than release-system app entries.

5. Apply document-specific rules:
