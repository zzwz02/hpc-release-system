"""Thin OpenAI SDK chat client for the local/intranet OpenAI-compatible endpoint.
...
def default_env_file() -> Path:
    return Path(__file__).resolve().parents[1] / "qa_llm.env"


def configured_env_file() -> Path:
    custom = os.environ.get("QA_LLM_ENV_FILE", "").strip()
    return Path(os.path.expandvars(custom)).expanduser() if custom else default_env_file()
...
def chat_json(system: str, user: str, *, timeout: int = 180, progress: Callable[[int], None] | None = None) -> str:
